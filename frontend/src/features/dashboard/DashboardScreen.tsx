...
import { Badge, StatusBadge, StatusIndicator, type StatusKey } from '@/components'
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2.35fr minmax(320px, 1fr)', gap: 'var(--space-4)', alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          <SectionCard>
            ...
            />
            {recents.map((i, idx) => (
              <button key={i.id} onClick={() => open(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', textAlign: 'left', border: 'none', borderTop: idx === 0 ? 'none' : '1px solid var(--divider)', background: 'transparent', cursor: 'pointer', padding: '11px 0' }}>
                <Badge tone="accent" size="sm">ISSUE</Badge>
