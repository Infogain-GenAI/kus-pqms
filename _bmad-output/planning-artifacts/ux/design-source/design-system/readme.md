...
> **Sync note (N-PQMS rebuild):** This file is a faithful mirror of the source design system from the "Kia N-PQMS V2-V3" Claude Design project, kept as reference input for the new build. Note that the source system includes severity/scoring components (`SeverityIndicator`, `SeverityBar`, "Severity auto-calculated") — **issue scoring is out of scope for the ISM rebuild** per the product brief and PRD, so those components/tokens are reference-only and are not to be built in this release.
